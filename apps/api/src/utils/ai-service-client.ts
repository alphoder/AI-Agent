...
export function aiServiceWsUrl(path: string): string {
  // Both live sockets can be served by the Cloudflare Worker while the HTTP
  // endpoints — scoring, briefs, plans — stay on the Python service. Only the
  // WebSockets move, and only when RELAY_WS_URL is set, so the cutover is one
  // variable and so is the way back.
  const useRelay = (path === '/ws/session' || path === '/ws/assistant') && config.RELAY_WS_URL;
