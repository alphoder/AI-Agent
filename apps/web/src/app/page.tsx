...
// Real product numbers — the same facts used across the app.
const STATS = [
  { value: '36', label: 'named people to call' },
  { value: '70+', label: 'languages to practise in' },
  { value: '5 min', label: 'one focused call' },
  { value: 'Free', label: 'while we are in beta' },
];
...
/** Visible focus ring shared by every interactive element on the page. */
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 backdrop-blur-md bg-background/70 border-b border-border">
        <div className="mx-auto max-w-6xl px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 font-semibold tracking-tight"><Mic className="h-5 w-5 text-primary" /> SpeakCoach</div>
          <nav className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
            <a href="#how" className={`hover:text-foreground transition-colors rounded-md ${FOCUS}`}>How it works</a>
            <a href="#features" className={`hover:text-foreground transition-colors rounded-md ${FOCUS}`}>Features</a>
            <a href="#scenarios" className={`hover:text-foreground transition-colors rounded-md ${FOCUS}`}>Scenarios</a>
          </nav>
          <Link href={dest} className={`rounded-full bg-foreground hover:bg-foreground/90 text-background px-4 py-2 text-sm font-medium transition-colors ${FOCUS}`}>
            Start free
          </Link>
        </div>
      </header>
      ...
      <section className="relative isolate flex min-h-[90vh] items-center overflow-hidden bg-background">
        ...
        <div aria-hidden className="absolute inset-0 -z-10 bg-[linear-gradient(100deg,hsl(var(--background)),hsl(var(--background))_32%,rgba(255,255,255,0.78)_56%,rgba(255,255,255,0.25)_72%,transparent_84%)]" />
        <div aria-hidden className="absolute inset-x-0 bottom-0 -z-10 h-28 bg-gradient-to-t from-background to-transparent" />
          ...
          <div className="max-w-2xl">
            <Reveal>
              <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl leading-[1.02] tracking-tight">
                Your{' '}
                <span key={moment} className="word-swap inline-block italic text-primary">{MOMENTS[moment]}</span>,
                <br />
                rehearsed.
              </h1>
            </Reveal>
            ...
            <Reveal delay={220}>
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link href={dest} className={`group inline-flex items-center gap-2 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-3 font-medium shadow-lg shadow-primary/20 transition-all hover:-translate-y-0.5 ${FOCUS}`}>
                  Start practicing <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
                </Link>
                <a href="#how" className={`inline-flex items-center gap-1.5 rounded-full border border-border bg-background/70 backdrop-blur px-6 py-3 font-medium text-foreground/80 hover:bg-background transition-colors ${FOCUS}`}>
                  How it works
                </a>
              </div>
            </Reveal>
            <Reveal delay={300}>
              <div className="mt-9 flex flex-wrap items-center gap-x-8 gap-y-2 text-sm text-muted-foreground">
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-20">
        <Reveal>
          <h2 className="font-display text-4xl sm:text-5xl tracking-tight text-balance text-center">This is what a practice call looks like.</h2>
          <p className="mx-auto mt-4 max-w-xl text-center text-muted-foreground leading-relaxed">Five minutes with a customer who has somewhere else to be. No slides, no role-play with your manager.</p>
        </Reveal>
        <Reveal delay={120}>
          <div className="mt-12">
            <SessionDemo />
      ...
      {/* By the numbers — quiet proof this is a real library, not a mockup */}
      <section aria-label="By the numbers" className="relative border-y border-border bg-muted/50">
        <div className="mx-auto max-w-6xl px-6 py-12 grid grid-cols-2 lg:grid-cols-4 gap-y-8 gap-x-6 text-center">
          {STATS.map((s, i) => (
            <Reveal key={s.label} delay={i * 80}>
              <div className="font-display text-4xl sm:text-5xl tracking-tight">{s.value}</div>
      ...
      <section className="relative mx-auto max-w-6xl px-6 py-24">
        <div className="grid items-center gap-14 lg:grid-cols-2">
          <Reveal>
            <div>
              <h2 className="font-display text-4xl sm:text-5xl tracking-tight leading-tight text-balance">You never call a stranger.</h2>
          ...
          <Reveal delay={120}>
            <div className="rounded-3xl border border-border bg-card p-7 shadow-[0_30px_80px_-40px_rgba(24,24,27,0.35)]">
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground/70">Client file</p>
              <p className="mt-2 font-display text-2xl tracking-tight">{FILE.name}</p>
              <p className="text-sm text-muted-foreground">{FILE.headline}</p>

              <dl className="mt-5 grid grid-cols-2 gap-x-6 gap-y-3 rounded-2xl bg-muted p-4">
                {FILE.facts.map((f) => (
                  <div key={f.label}>
                    <dt className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{f.label}</dt>
                    <dd className="mt-0.5 text-sm text-foreground/80">{f.value}</dd>
                  </div>
                ))}
              </dl>

              <p className="mt-5 text-sm leading-relaxed text-muted-foreground">{FILE.life}</p>

              <div className="mt-5 rounded-2xl border border-dashed border-border p-4">
                <p className="text-sm font-semibold text-foreground/80">Not on file</p>
                <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                  {FILE.unknowns.map((u) => <li key={u}>{u}</li>)}
                </ul>
                <p className="mt-3 text-xs text-muted-foreground/70">You only find this out by asking well on the call.</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      {/* How it works */}
      <section id="how" className="relative bg-muted border-y border-border">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <Reveal><h2 className="font-display text-4xl sm:text-5xl tracking-tight text-balance text-center">How it works</h2></Reveal>
            ...
            {STEPS.map((s, i) => (
              <Reveal key={s.n} delay={i * 100}>
                <div className="relative flex flex-col items-center text-center">
                  <div className="relative z-10 grid h-14 w-14 place-items-center rounded-full bg-primary font-display text-xl text-primary-foreground shadow-lg shadow-primary/30 ring-8 ring-muted">{i + 1}</div>
                  <h3 className="mt-6 text-xl font-semibold">{s.title}</h3>
                  <p className="mt-2 max-w-xs text-muted-foreground leading-relaxed">{s.body}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section id="features" className="relative mx-auto max-w-6xl px-6 py-24">
        <Reveal><h2 className="font-display text-4xl sm:text-5xl tracking-tight text-balance text-center">Built to close more, mis-sell less</h2></Reveal>
                ...
                <TiltCard className={`h-full rounded-2xl border p-7 ${tone}`}>
                  <div className={wide ? 'flex items-center gap-6' : ''}>
                    {bixyTile ? (
                      ...
                      <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${filled ? 'bg-white/15 text-white' : 'bg-blue-50 text-primary'}`}>
                        <f.icon className="h-5 w-5" />
                      </div>
                    )}
                    <div>
                      <h3 className={`font-semibold ${wide ? 'text-xl' : 'mt-5'}`}>{f.title}</h3>
                      <p className={`mt-1.5 text-sm leading-relaxed ${filled ? 'text-blue-50' : 'text-muted-foreground'}`}>{f.body}</p>
                    </div>
                  </div>
                </TiltCard>
              </Reveal>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-24">
        <Reveal><h2 className="font-display text-4xl sm:text-5xl tracking-tight text-balance text-center">Real calls, rehearsed first.</h2></Reveal>
        <div className="mt-16 grid md:grid-cols-2 gap-6">
          {[
            ...
            <Reveal key={m.title} delay={i * 100}>
              <TiltCard className="group overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
                {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {/* Scenarios */}
      <section id="scenarios" className="relative bg-muted border-y border-border">
        <div className="mx-auto max-w-6xl px-6 py-24 grid lg:grid-cols-2 gap-14 items-center">
          <Reveal>
            <div>
              <h2 className="font-display text-4xl sm:text-5xl tracking-tight leading-tight text-balance">Thirty-six people to call.</h2>
              <p className="mt-5 text-muted-foreground leading-relaxed">A term-life cold call, a copay dispute, a CFO who has heard it all before. Every one is a named person with a file, sorted by what you are trying to fix. Browse them, or let the dice pick.</p>
              <div className="mt-7 flex flex-wrap gap-2">
                {CATEGORIES.map((c) => (
                  <span key={c} className="rounded-full border border-border bg-card px-3.5 py-1.5 text-sm text-muted-foreground">{c}</span>
                ))}
              </div>
            </div>
          </Reveal>
          <Reveal delay={120}>
            <TiltCard>
              ...
              <img src="/landing/scenarios.png" alt="Browse practice scenarios"
                className="w-full rounded-3xl border border-border shadow-[0_30px_80px_-30px_rgba(24,24,27,0.35)]" />
            </TiltCard>
          </Reveal>
        </div>
      </section>
      ...
      <section className="relative mx-auto max-w-6xl px-6 py-24">
        <Reveal>
          <div className="rounded-3xl border border-border bg-card p-10 flex flex-col md:flex-row items-center gap-8 shadow-sm">
            <div className="relative shrink-0">
              <div aria-hidden className="bixy-halo absolute inset-0 m-auto h-32 w-32" />
              <AssistantOrb state="listening" size={140} />
            </div>
            <div>
              <h2 className="font-display text-4xl tracking-tight text-balance">Meet Bixy.</h2>
              <p className="mt-3 text-muted-foreground max-w-xl leading-relaxed">Tap the orb anywhere and simply talk: “show my scenarios”, “open my reports”, “I keep losing them on price”. Bixy finds the call that fixes it and hands it to you, ready to start.</p>
            </div>
          </div>
        </Reveal>
      </section>
      ...
      <section className="relative bg-zinc-900 text-white overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-28 text-center">
          <Reveal>
            <h2 className="font-display text-5xl sm:text-6xl tracking-tight text-balance">Your next sales call, rehearsed.</h2>
            ...
            <Link href={dest} className="mt-10 inline-flex items-center gap-2 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 text-lg font-medium transition-all hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-900">
      ...
      <footer className="relative border-t border-border py-10 text-center text-sm text-muted-foreground">
        <div className="flex items-center justify-center gap-2 mb-2 text-foreground"><Mic className="h-4 w-4 text-primary" /> SpeakCoach</div>
